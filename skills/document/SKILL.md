...
2. Refresh the **architecture doc** (inline Mermaid-C4 L1/L2) in the **same item** when the change moves
   system/container structure — STABLE, changes only with the code (D41). *(Read its location from the
   project layout / `config.json`; don't hard-code — the docs-root is still open.)*
...
## Audit mode (retention + prune — D61)
...
- **Deletion-test (judgment)** over `CLAUDE.md` + `rules/` — cut prose the agent no longer needs; bloat makes
  it ignore its own instructions (D41).
- **Dead-node prune:** a deleted source file → delete its `docs/knowledge/` node.
Distillation (postmortems → lessons) is deferred (D61).
